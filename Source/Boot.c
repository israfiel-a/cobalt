...
static EFI_STATUS waitKey(uint64_t timeOut, EFI_SYSTEM_TABLE *table)
{
    ...
    table->BootServices->CreateEvent(EVT_TIMER, TPL_APPLICATION, nullptr,
                                     nullptr, &timerEvent);
    table->BootServices->SetTimer(timerEvent, 1, 10000000);
    ...
    {
        UINTN index;
        table->BootServices->WaitForEvent(1, &timerEvent, &index);
        ...
        {
            table->BootServices->SetTimer(timerEvent, 0, 0);
                           ...
                           EFI_SYSTEM_TABLE *SystemTable)
{
    ...
    EFI_STATUS keyStatus = waitKey(5, SystemTable);
